...
def h_registration(r, path_project): # do naprawy też
    registration = locate_image_center('Registration.jpg', path_project, r)

    if registration:
        pg.sleep(3.0)
        reserved = locate_image_center('Reserved.jpg', path_project, r)

        if reserved:
            box = locate_image_center('Box.jpg', path_project, r)
            pg.sleep(3)
        else:
    ...
    #Pierwsza zapisana opcja rejestracji i nocleg za 1200 tylko łapie 
        # fav_reg = locate_image_center('favreg.jpg', path_project, r)
        # pg.move(0,20)
        # pg.click()
        # pg.sleep(1.5)
        # r_price = locate_image_center('Price.JPG', path_project, r)
...
def cancel_reg(r, path_project):
    pg.sleep(1.5)
    cancel = locate_image_center('cancel.JPG', path_project, r)
    if cancel:
        pg.press('enter')


def h_multiplication( r, path_project):
    reproduce_check = locate_image('mating_mares.JPG', path_project, r)
    if reproduce_check:
        pg.sleep(3)
        repro_img_tab = ['price_repro.JPG', 'select_horse.JPG', 'mating.JPG']
        for img in repro_img_tab:
            for i in range(10):
                repro_check = locate_image(img, path_project, r)
                if repro_check:
                    pg.sleep(1.5)
                    break
...
def account_quiting( r, path_project, quit_game):
    ...
    if "w" in quit_game: #wyjście ze współki
        share_account = locate_image('account_share.JPG', path_project, r)
        if share_account:
            exit_share = locate_image('exit_share.JPG', path_project, r)

    if "k" in quit_game: #wyjście z własnego konta
        pg.sleep(2)
        account = locate_image('account.JPG', path_project, r)

        if account:
            turn_share = locate_image('turn_share.JPG', path_project, r)
            if turn_share:
                pg.sleep(1)
                agree = locate_image('agree.JPG', path_project, r)
